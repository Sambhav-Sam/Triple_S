...
router.post("/userpic", async (req, res) => {
  try {
    ...
    //getting users data
    const data = await UserDetail.aggregate([{
        $match: {
          _id: {
            $ne: user._id
          }
    ...
    const mycoordinates = await UserDetail.findOne({
      _id: user._id
    }).select({
      location: 1
    });
...
router.post("/likeuser", async (req, res) => {
  try {
    ...
    const otherUser = await UserDetail.findOne({
      _id: likedUserId
    }).select({
      likedUser: 1
    });
...
router.post("/saveuser", async (req, res) => {
  try {
    ...
    const viewedUsers = await UserDetail.findOneAndUpdate({
      _id: user._id
    }, {
      $addToSet: {
        viewedUser: saveUserId
      }
    });
